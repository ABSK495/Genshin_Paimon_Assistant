...
目前已经更新到0.1.4版本，更新内容可查看压缩包内的更新日志。派懵可以直接在github仓库、蓝奏云以及QQ群进行下载，有问题随时联系作者。蓝奏云下载地址：https://wwsn.lanzoue.com/b03x0kjij 密码:shfy
...
开启自动钓鱼功能后，只需要手动抛竿，派懵会自动完成拉条和收杆的操作。适用于游戏内所有种类的鱼例如雷鸣仙、假龙类、炮鲀类、雪中君、渊下宫鳐鱼、枫丹机械鱼以及它们的观赏鱼类型。

你可以使用派懵帮助你完成鱼叉、竭泽、灰河渡手等钓鱼武器的获取，或者是用于完成通通400铃的成就等。目前自动钓鱼的代码存在问题，精度低并且会出现无法使用的情况，暂时不推荐使用。
...
### 11. 派懵浏览器，攻略查找视频跟跑
在「浏览器」选项卡中点击「打开浏览器」即可启用派懵浏览器。初次启动会打开派懵演示地址，此时需要在派懵界面粘贴需要查看的视频网址，点击加载网址即可跳转。

派懵内置了针对B站视频攻略跟跑的快捷键，其中波浪键~用于暂停和播放，可以在游戏内按下而不中断当前操作，但需要保证当前没有音乐播放器正在播放。

数字键5进度条后退，6进度条前进，7减小不透明度，8增加不透明度，9隐藏恢复窗口。

部分用户打开派懵会出现红屏的情况，此时可以尝试安装微软的webview2库，但有可能安装了还是红屏，需要后续修复。

webview2库下载地址：https://developer.microsoft.com/zh-cn/microsoft-edge/webview2/，通常下载独立安装程序的x64版本。

派懵浏览器目前点击部分链接会使用外部的默认浏览器打开，已经联系库的开发者添加控制功能，但预计需要很久才能上线。
